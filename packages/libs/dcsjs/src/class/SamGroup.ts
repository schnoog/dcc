...
import { GroundGroup, GroundGroupProps } from "./GroundGroup";
import { Mission } from "./Mission";

export class SamGroup extends GroundGroup {
  ...
  public override toGenerated(
    mission: Mission
  ): Data.GeneratedTypes.GroundGroup {
    let target: Data.Target | undefined = undefined;
    if (this.objectiveName != null) {
      const targets =
        Data.Theatres[mission.theatre].targets[this.objectiveName];

      if (targets == null) {
        throw new Error(`targets ${this.objectiveName} not found`);
      }

      target = randomItem(targets, (t) => t.type === "SAM");
    }

    const groupPosition = target == null ? this.position : target.position;

    const units: Data.GeneratedTypes.GroundUnit[] = this.units.map(
      (unit, i) => {
        let position: Data.Position | undefined =
          target == null ? undefined : target.unitPositions[i];
