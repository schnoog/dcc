...
export class Airdrome {
	...
	get airdromeDefinition() {
		const definition = Data.Theatres[this.#theatre].airdromeDefinitions[this.#name];
	...
	public reserveStand(isHelicopter: boolean, from: number, to: number) {
		const usableStands = isHelicopter ? this.#stands.filter((stand) => stand.helicopter) : this.#stands;

		let availableStands = usableStands.filter((stand) => stand.inUse.length === 0);
